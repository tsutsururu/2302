...
mex : 区間に含まれない & 最小 ⇒ 区間に含まれる値は ∞ として全区間を取得すれば mex が求められるのでセグ木でも解ける

単位元 ∞ , 更新関数 min としたセグメントツリーを作成、i が含まれれば ∞ ,含まれなければ i をたてる。これによって[0,N+10) 区間取得で mex が求まる。 区間に新たに含まれる要素は必ずセグ木を更新。あふれる要素は他に同じ値がなければ更新する。
